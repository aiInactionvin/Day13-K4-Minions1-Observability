...
- Ba bước kiểm tra đầu tiên:
  1. Kiểm tra Dashboard panel "Latency percentiles" và bảng "Investigation queue" để lấy correlation ID chậm nhất.
  2. Mở Langfuse UI Traces theo session/correlation metadata để tìm span có duration cao bất thường (RAG retrieval hay LLM generation).
...
- Ba bước kiểm tra đầu tiên:
  1. Kiểm tra Dashboard panel "Error rate and breakdown".
  2. Lọc các dòng log trong `data/logs.jsonl` có `event=request_failed` hoặc `level=error`.
...
- Ba bước kiểm tra đầu tiên:
  1. Kiểm tra Dashboard panel "Quality proxy".
